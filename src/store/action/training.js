...
export const updateTraining= ( date,nbHour,place,price,title,mode,list_instructor,id ) => {
    return async (dispatch,getState) =>{
        const token = getState().auth.token;
        let formData  = new FormData();
        formData.append("date", date);
        ...
        formData.append("type_payement",mode.value);
     //   formData.append("formateurs",JSON.stringify(list_instructor));
        formData.append("formateurs[]",""+list_instructor);
        formData.append("_method", "put");
  
        const response = await axios({
            method: "POST",
            url: `http://127.0.0.1:8000/api/formation/update/${id}`,
             data: formData,
             headers: {"Authorization" :`Bearer ${token}`} ,
             "Content-Type": "multipart/form-data" 
            })
...
export const SearchTraining= (name) => {
    return async (dispatch,getState) =>{
        const token = getState().auth.token;
     
     dispatch({type:SEARCHTRAINING,payload:{name,}});
